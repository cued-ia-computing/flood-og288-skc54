    ...
    for i in stations:
        if type(i.relative_water_level()) == float:  # Check to see if the data is corrct and can be used
            ...
            if len(station_levels[0]) != 0 or len(station_levels[1]) != 0:
                p = polyfit(station_levels[0], station_levels[1], 1)[0][0] #This takes the coefficient of the polynomial produced by polyfit. As it is order one, the coefficient is the slope. This can tell us if the water level si rising or decreasing
                if i.relative_water_level() > 3 and p > 0:  # If the relative water level is above 3, the risk is SEVERE
                ...
                elif i.relative_water_level() > 2 and p > 0:  # If the relative water level is above 2, the risk is HIGH
                ...
                elif i.relative_water_level() > 1 and p > 0:  # If the relative water level is above 1, the risk is MODERATE
                ...
                else:  # If the relative water level is under 1, the risk is LOW
